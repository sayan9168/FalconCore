...
#[cfg(test)]
mod tests {
    use super::{capability::{Capability,CapabilityPolicy,CapabilityRegistry}, compiler::Compiler, diagnostics::{Diagnostic, Severity}, lexer::Lexer, parser::Parser, symbols::analyze_module, typecheck::{Type, TypeChecker}, vm::VM};
    ...
    #[test]fn symbol_analysis_tracks_exports(){let ast=parse("export secure const answer = 42");let m=analyze_module("math",&ast).unwrap();assert!(m.symbols["answer"].exported);}
    #[test]fn capabilities_are_deny_by_default(){let mut r=CapabilityRegistry::new(CapabilityPolicy::deny_all());assert!(r.request(Capability::NetworkScan).is_err());}
}