...
mod tests {
    ...
    fn run(source: &str) -> Result<(), super::vm::RuntimeError> {
        ...
        VM::with_functions(compiler.get_constants().to_vec(), compiler.get_code().to_vec(), compiler.get_functions().clone()).run()
    }
    ...
    #[test]
    fn recursive_functions() { run("fn count(n){ if n > 0 { print n return count(n-1) } return 0 } count(3)").unwrap(); }
    #[test]
    fn division_by_zero() { assert!(matches!(run("print 10 / 0"), Err(super::vm::RuntimeError::DivisionByZero))); }

    #[test]
    fn type_checker_accepts_numeric_program() {
        let ast = parse("let x = 10 print x + 2");
        let mut checker = TypeChecker::new();
        checker.check(&ast).unwrap();
        assert_eq!(checker.variables().get("x"), Some(&Type::Int));
    }

    #[test]
    fn type_checker_rejects_bad_repeat() {
        let ast = parse("repeat \"nope\" { print 1 }");
        let mut checker = TypeChecker::new();
        assert!(checker.check(&ast).is_err());
    }

    #[test]
    fn type_checker_rejects_unknown_function() {
